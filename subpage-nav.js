  ...
  if (!document.getElementById(STYLE_ID)) {
    ...
    style.textContent = `
      ...
      .nav-item > a::before {
        ...
        height: 2px;
        border-radius: 999px;
        background: linear-gradient(90deg, var(--brand-1, #79c5c7), var(--brand-2, #2ea6d4));
        opacity: 0.46;
      ...
      .site-header.menu-open .nav-item > a {
        ...
        border-left: 2px solid rgba(121, 197, 199, 0.55);
        padding-left: calc(0.62rem - 2px);
      }
      .site-header.menu-open .nav-item > a:hover {
        color: #ffffff;
        background: rgba(46, 166, 212, 0.2);
        border-left-color: rgba(46, 166, 212, 0.72);
      }
      .site-header.menu-open .nav-item > a::before {
        display: none;
      }
  ...
  const navModel = [
    {
      ...
      mega: true,
      children: [
        { kind: "heading", label: "Systems", href: "/services/systems.html", tone: "systems" },
        ...
        { kind: "divider" },
        { kind: "heading", label: "Services", href: "/services/services.html", tone: "services" },
    ...
    const children = item.children
      .map((child) => {
        if (child.kind === "heading") {
          const headingClass = `dropdown-section-title${child.tone ? ` dropdown-section-title--${child.tone}` : ""}`;
          if (child.href) {
            return `<a role="menuitem" class="${headingClass}" href="${child.href}">${child.label}</a>`;
          }
          return `<span class="${headingClass}">${child.label}</span>`;
        }
